...
template <class CodecType, class FrameSinkType> struct StreamContext {
  ...
  void CleanUp() {
    ...
    if (codec != nullptr)
      delete codec;
    // the demuxer can be shared amongst different contexts, so we don't delete
    // them here
  }
};
...
void *ffmpegCppCreate(const char *outputFileName) {
  ...
  try {
    ...
    SetError(ctx, string("Failed to create output file " + ctx->outputFileName +
                         ": " + string(e.what())));
    return nullptr;
  }
}
...
void ffmpegCppAddVideoStream(void *handle, const char *videoFileName) {
  ...
  try {
    ...
    if (ctx->videoContext.demuxer == nullptr) {
      ctx->videoContext.demuxer =
          new Demuxer(ctx->videoContext.sourceFileName.c_str());
      ctx->uniqueDemuxers.push_back(ctx->videoContext.demuxer);
    }

    // create the encoder
    ctx->videoContext.codec =
        new VideoCodec(ctx->muxer->GetDefaultVideoFormat()->id);
    ctx->videoContext.encoder =
        new VideoEncoder(ctx->videoContext.codec, ctx->muxer);
  } catch (FFmpegException e) {
    SetError(ctx, string("Failed to add video stream " + string(videoFileName) +
                         ": " + string(e.what())));
  }
}
...
void ffmpegCppAddAudioStream(void *handle, const char *audioFileName) {
  ...
  try {
    ...
    if (ctx->audioContext.demuxer == nullptr) {
      ctx->audioContext.demuxer =
          new Demuxer(ctx->audioContext.sourceFileName.c_str());
      ctx->uniqueDemuxers.push_back(ctx->audioContext.demuxer);
    }

    // create the encoder
    ctx->audioContext.codec =
        new AudioCodec(ctx->muxer->GetDefaultAudioFormat()->id);
    ctx->audioContext.encoder =
        new AudioEncoder(ctx->audioContext.codec, ctx->muxer);
  } catch (FFmpegException e) {
    SetError(ctx, string("Failed to add audio stream " + string(audioFileName) +
                         ": " + string(e.what())));
  }
}
...
void ffmpegCppGenerate(void *handle) {
  ...
  try {
    ...
    if (ctx->videoContext.hasFilter) {
      ctx->videoContext.filter =
          new Filter(ctx->videoContext.filterString.c_str(), videoFrameSink);
      videoFrameSink = ctx->videoContext.filter;
    }
    if (ctx->audioContext.hasFilter) {
      ctx->audioContext.filter =
          new Filter(ctx->audioContext.filterString.c_str(), audioFrameSink);
    ...
    SetError(ctx,
             string("Failed to generate output file: " + string(e.what())));
  }
}

bool ffmpegCppIsError(void *handle) { return ((Context *)handle)->errored; }
...
void ffmpegCppClose(void *handle) { CleanUp((Context *)handle); }
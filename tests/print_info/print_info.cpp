  ...
  try {
    ...
    for (int i = 0; i < info.audioStreams.size(); ++i) {
      ...
      cout << "Stream #" << (i + 1) << ": codec " << stream.codec->name
           ...
           << demuxer->GetFrameCount(stream.id) << " frames" << endl;
    }

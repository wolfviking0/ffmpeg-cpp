...
int main() {
  ...
  try {
    ...
    const char *rawAudioFile = strdup(
        (std::string(DATA_FOLDER) +
         "/Vivaldi_s16le_2_channels_samplerate_11025.dat")
            .c_str());
    ;
    const char *rawAudioFormat = "s16le";
    int rawAudioSampleRate = 11025;
    int rawAudioChannels = 2;
    RawAudioFileSource *audioFile =
        new RawAudioFileSource(rawAudioFile, rawAudioFormat, rawAudioSampleRate,
                               rawAudioChannels, encoder);

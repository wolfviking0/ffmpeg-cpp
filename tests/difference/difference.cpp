...
int main() {
  // This example will apply a vignette filter and then compare the changed
  // output to the original video by using the difference filter.
  try {
    ...
    // Load both video's
    Demuxer *videoContainer1 =
        new Demuxer((std::string(DATA_FOLDER) + "/carphone.h264").c_str());
    Demuxer *videoContainer2 =
        new Demuxer((std::string(DATA_FOLDER) + "/carphone.h264").c_str());
    ...
    while (!videoContainer1->IsDone() || !videoContainer2->IsDone()) {
      if (!videoContainer1->IsDone())
        videoContainer1->Step();
      if (!videoContainer2->IsDone())
        videoContainer2->Step();
    }

...
// to make it work. Look at the examples in the Codecs-dir to see how it is
// done.
void PlayDemo(int argc, char **argv) {

  // These are example video and audio sources used below.
  const char *rawVideoFile = strdup((std::string(DATA_FOLDER) + "/carphone_qcif.y4m").c_str());
  int rawVideoWidth = 176;
  int rawVideoHeight = 162;
  const char *rawAudioFile = strdup((std::string(DATA_FOLDER) + "/Vivaldi_s16le_2_channels_samplerate_11025.dat").c_str());
  const char *rawAudioFormat = "s16le";
  int rawAudioSampleRate = 11025;
  int rawAudioChannels = 2;

  const char *encodedVideoFile = strdup((std::string(DATA_FOLDER) + "/carphone.h264").c_str());
  const char *encodedAudioFile = strdup((std::string(DATA_FOLDER) + "/Vivaldi_Sonata_eminor_.mp3").c_str());

  const char *containerWithVideoAndAudioFile = strdup((std::string(DATA_FOLDER) + "/big_buck_bunny.mp4").c_str());
  const char *containerWithAudioFile = strdup((std::string(DATA_FOLDER) + "/DesiJourney.wav").c_str());

  // hard-code the settings here, but let them be overridden by the arguments
  string inputAudioSource =
      "CONTAINER"; // options are RAW, ENCODED, CONTAINER, GENERATED
  string inputVideoSource =
      "ENCODED"; // options are RAW, ENCODED, CONTAINER, GENERATED
  string outputAudioCodec = "NONE"; // options are MP2, AAC, NONE
  string outputVideoCodec = "H264"; // options are H264, H265, VP9, NONE (H264
                                    // and H265 only work on Nvidia hardware)
  string outputContainerName = "out.mp4"; // container format is deduced from
                                          // extension so use a known one

  // you can use any filter string that you can use in the ffmpeg command-line
  // here set the filter to NULL to disable filtering. See
  // https://trac.ffmpeg.org/wiki/FilteringGuide for more info This example
  // rotates the entire video and then puts a vignette on top of it.
  ...
  try {
    ...
    // create an encoder - this encoder will receive raw data from any source
    // (filter, file, container, memory, etc), encode it and send it to the
    // muxer (the output container).
    ...
    // this encoder will receive data from an input source (file, raw, filter,
    // etc), encode it and send it to the output container (muxer)
    ...
    // only do this when there is an output - otherwise there is no point in
    // reading audio
    InputSource *audioInputSource = nullptr;
    if (audioEncoder != nullptr) {
      if (inputAudioSource == "RAW") {
        printf("Pulling audio from %s...\n", rawAudioFile);
        audioInputSource = new RawAudioFileSource(
            rawAudioFile, rawAudioFormat, rawAudioSampleRate, rawAudioChannels,
            audioEncoder);
      } else if (inputAudioSource == "ENCODED") {
        printf("Pulling audio from %s...\n", encodedAudioFile);
        audioInputSource = new EncodedFileSource(encodedAudioFile,
                                                 AV_CODEC_ID_MP3, audioEncoder);
      } else if (inputAudioSource == "CONTAINER") {
        // if the input comes from a container, we use the demuxer class - it is
        // just an input source like any other
    ...
    // Instead of feeding the video source directly to the encoder, we feed it
    // to the video filter instead, which will pass it on to the encoder.
    ...
    if (videoEncoder != nullptr) {
      if (inputVideoSource == "RAW") {
        ...
        printf("Pulling video from %s...\n", encodedVideoFile);
        videoInputSource =
            new RawVideoFileSource(encodedVideoFile, videoFrameSink);
    ...
    // As an awkward but necessary first step, we need to prime each input
    // source. This step decodes a part of the input data and from this,
    // generates info about the stream and propagates it all the way to the
    // output container, which needs to write this data to its header.
    if (videoInputSource != nullptr)
      videoInputSource->PreparePipeline();
    if (audioInputSource != nullptr)
      audioInputSource->PreparePipeline();

    // finally, we can start writing data to our pipelines. Open the floodgates
    // to start reading frames from the input, decoding them, optionally
    // filtering them, encoding them and writing them to the final container.
    // This can be interweaved if you want to.
    if (audioInputSource != nullptr) {
      while (!audioInputSource->IsDone())
        audioInputSource->Step();
    }
    if (videoInputSource != nullptr) {
      while (!videoInputSource->IsDone())

...
public:
  ...
  virtual void Close(int streamIndex) { delete stream; }

  virtual bool IsPrimed() {
    // Return whether we have all information we need to start writing out data.
    // Since we don't really need any data in this use case, we are always
    // ready. A container might only be primed once it received at least one
    // frame from each source it will be muxing together (see Muxer.cpp for how
...
int main() {
  // This example will decode a video stream from a container and output it as
  // raw image data, one image per frame.
  try {
    // Load this container file so we can extract video from it.
    Demuxer *demuxer =
        new Demuxer((std::string(DATA_FOLDER) + "/big_buck_bunny.mp4").c_str());

    // Create a file sink that will just output the raw frame data in one PGM
    // file per frame.
